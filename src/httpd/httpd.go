...
func handler(w http.ResponseWriter, r *http.Request) {
	...
	//获取配置文件
	t0 := time.Now()
	cfg := Config.FindByBoth(parseQuerys(r), r.URL.Path)
...
func Run(c config.ConfigFileT) {
	//初始化配置文件
	//TODO 需要修改配置文件路径
  fmt.Println(os.Args[0])
	Config = c
	fmt.Printf("start@ %s:%d %v \n", Config.Listen.Host, Config.Listen.Port, time.Now())
	http.HandleFunc("/", handler)
	if err := http.ListenAndServe(fmt.Sprintf("%s:%d", Config.Listen.Host, Config.Listen.Port), nil); err != nil {
		log.Fatal(err)
	}
}
